...
| 편중 ← 예산 규모 | 간단 규모(문서 4건)에서 구조적으로 걸린다. 절제 실험은 예산을 바꾸지 않아 검증 못 했다. 단일에서 배정·구역을 끄면 0.25→**0.53** | **약함** — 예산 매핑은 미검증 |
...
같은 설정도 시행마다 근거율이 **최대 21%p** 움직인다(다갈래 기본 0.72–0.93) — 칸마다 3회씩 재서 판정했다(`tools/반복분석.py`).
...
**3회씩 다시 쟀다** (칸마다 2회 더 · 24편 · ₩829 · `python -m pipeline.repeat` → `tools/반복분석.py`).
- **다 켠 기본만 그물을 9/9 통과**했다. 끈 설정은 5~8/9 — 허위 인용·인용 0곳 절·편중이 울리기 시작한다.
- 같은 칸도 근거율이 **최대 21%p** 흔들린다(다갈래 기본 0.72–0.93). 위 표의 차이 대부분이 이 폭 안이다.
- **역할을 끈 쪽이 낫다는 관찰은 사라졌다** — 3회 평균은 ↑↑↓, 범위가 전부 겹친다.
- 남은 것: 재위임을 끄면 인용 0곳 절(2/9, 다른 설정 0/27) · 단일에서 배정·구역을 끄면 편중 0.25→0.53.
- 표와 판정 전문은 [`REPORT.md`](REPORT.md) 4.3.
