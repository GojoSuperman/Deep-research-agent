...
- ⏸️ **7 배포 ← 다음 할 일.** 설정·도구는 다 됐다(커밋 `f34136b`). **관문은 계정 확인 하나.**
  - Vercel 대시보드에서 `seatBlock: TEAM_ACCESS_REQUIRED` 확인 배너를 처리해야 빌드가 돈다
  - 함수 시간 상한은 **관문이 아니다** — Hobby 도 300초다(옛 60초 아님). 정식 규모 40초대는 여유
  - 배포본에서 **라이브 API 는 이미 검증했다**: 18.5초 · ₩10 · 근거율 1.00 · 키 유출 0건
  - 남은 확인은 **정적 파일이 200 인지** 하나 — 지난 배포는 `framework:python` 으로 잡혀 전부 405 였다
  - `git push` 가 곧 배포다 (저장소가 연결돼 있다). 검증은 `bash tools/배포검증.sh <URL> <키>`
