...
| 인용 서고 ← 배정 | 팀 4·4·3 vs 혼자 3·3·3 (관계·다갈래·단일). 그런데 **역할을 끈 관계 편이 6개로 가장 넓고, 핵심 문서는 0.25** — 넓게 흩어진 것이 헤맨 것일 수도 있다 | **약함** — 팀/혼자는 가르지만 많을수록 좋은 것이 아니다 |
...
(`tools/서고커버리지.py`). 녹화 15편의 보고서로 다시 쟀다 — 새로 돌리지 않았다.

| 질문 | 인용 서고 수 (팀/혼자) | 핵심 문서 도달 (팀/혼자) | 인용이 0곳인 필수 서고 (팀/혼자) |
|---|---|---|---|
| 단일 | 3 / 3 | 0.50 / 0.50 | 없음 / 없음 |
| 다갈래 | **4 / 3** | **0.75 / 0.50** | 측정 / 측정·유사사례 |
| 관계 | **4 / 3** | **0.75 / 0.00** | 편향 / **관계** |

나눌 필요 없는 단일 질문에서는 차이가 없고, 흩어진 질문일수록 벌어진다 — 1장 질문 표에 적은
「왜 나눌 만한가」와 같은 방향이다.
**단 핵심 문서와 필수 서고는 `data/questions.json` 에 적은 목록**이고, 그 목록은 녹화 다음 날
적었다. 결과를 본 사람이 쓴 목록으로 결과를 채점하는 순환이 될 수 있어 **지표(`metrics.py`)에
넣지 않고 도구에서 참고로만** 잰다. 지표는 정답표를 쓰지 않는다는 원칙(3장)도 지킨다.
...
| 무엇 | 왜 |
|---|---|
| **「서고를 고르게 밟았는가」 — 절반 했다** | 인용 서고 수를 지표에 넣고 도구를 붙였다(4.2). 남은 것: 인용 서고 수는 **많을수록 좋은 게 아니라** 세기가 약하고, 팀/혼자를 크게 가르는 핵심 문서 도달은 **녹화 뒤에 적은 목록**에 기댄다. 새 질문을 녹화하기 **전에** 핵심 문서를 적어 두고 재야 믿을 수 있다 |
