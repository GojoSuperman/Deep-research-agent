...
**한 건 한 건은 잡음 폭 안이다.** 같은 팀 기본 설정을 3번 돌리면 근거율이 단일 0.57–0.73,
다갈래 0.72–0.93으로 흔들린다(4.3). 혼자의 0.69·0.84는 **그 범위 안**이다 — 범위를 확실히 벗어난
것은 관계(0.29)뿐이다. 그래도 유형 예측이 걸린 7건(다갈래 4 · 단일 3)이 **모두** 예측 쪽으로
갔다는 것은 남는다. 한 건씩은 믿기 어렵지만 일곱이 한쪽으로 모였다 — 그만큼만 말한다.
혼자 쪽의 흔들림은 아직 재지 않았다(7장).
...
| **n을 늘려 재기 — 절제 실험은 했다** | 칸마다 3회로 늘렸다(4.3). 남은 것: **혼자 대조군도 여러 번** 돌려야 한다. 팀은 3회 범위가 최대 21%p인데 혼자의 흔들림은 아직 모른다 — 4.2의 팀/혼자 비교는 양쪽 다 흔들림을 알아야 판정된다 |
